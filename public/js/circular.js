...
/* ════════════════════════════════════════════════════
   ...
   sobre el markup ya presente en circular.html.
   ════════════════════════════════════════════════════ */
...
/* Fecha corta con año a 2 dígitos, usada en los chips de "Circulares
    ...
    <tr>
      ...
      <td class="td-acciones">
        <div class="fila-acciones">
          ...
          <button class="btn-fila-eliminar" onclick="eliminarFila(${r.id}, '${r.no_circular}')">
...
async function cargarLibres() {
  try {
    const res = await fetch(`${API}/circular/liberados`, { headers: { 'Authorization': `Bearer ${TOKEN}` } });
    if (res.status === 401) { cerrarSesion(); return; }
    const data = await res.json();
    if (!res.ok) throw new Error(data.mensaje || 'Error al cargar los circulares libres.');
    LIBRES = data;
    pintarLibres();
  } catch { /* silencioso: no bloquea la tabla principal */ }
}

/* El panel de Circulares Libres arranca colapsado (solo el título con el
   ...
   refresque (p. ej. tras "Circular Libre del Día" o un Eliminar). */
...
function abrirNuevo() {
  EDITANDO_ID = null;
  document.getElementById('modal-nuevo-titulo').textContent = 'Nuevo No. Circular';
...
function editarFila(id) {
  ...
  EDITANDO_ID = id;
  document.getElementById('modal-nuevo-titulo').textContent = `Editar No. Circular ${r.no_circular}`;
...
async function guardarNuevo() {
  ...
  if (!editando && modo === 'anterior' && !libre) {
    errorEl.textContent = 'Selecciona un número de la lista de Circulares Libres.';
    return;
  }
  ...
  try {
    const url    = editando ? `${API}/circular/${EDITANDO_ID}` : `${API}/circular`;
    const method = editando ? 'PUT' : 'POST';
    const body   = editando
      ? { fecha, hora, a_quien_se_dirige: dirige, area_solicitante: area, solicitante: solicita, asunto }
      : { modo, no_circular: modo === 'anterior' ? libre : undefined,
    ...
    await cargarLibres();
    await sbisAlert({
      titulo: editando ? `No. Circular ${data.no_circular} actualizado` : `No. Circular ${data.no_circular} registrado`,
...
async function eliminarFila(id, numero) {
  const ok = await sbisConfirm({
    titulo: `¿Eliminar el No. Circular ${numero}?`,
  ...
  try {
    const res = await fetch(`${API}/circular/${id}`, {
      method: 'DELETE',
