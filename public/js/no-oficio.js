...
/* Desplegable (datalist) de "A quién se dirige": catálogo curado y fijo
   de personas (no se arma a partir de REGISTROS). Sigue permitiendo
...
function pintarOpcionesDirige() {
  ...
  if (!dl) return;
  const valores = CATALOGO_DIRIGE.slice().sort((a, b) => a.localeCompare(b, 'es'));
...
/* Agrupa los números liberados por mes (mes + año), usando la fecha
   en que se liberaron. Sirve tanto para el panel de "Oficios Libres"
   como para el <select> de "Asignar Anteriores": con cientos de
   números, ubicarlos sueltos era poco práctico, así que se organizan
   cronológicamente (enero, febrero, ...) y los que no traen una
   fecha válida quedan al final bajo "Sin fecha". */
  ...
  sel.innerHTML = '<option value="">— Selecciona un número —</option>' +
    grupos.map(g => `
      <optgroup label="${g.etiqueta}">
        ${g.items.map(l => `<option value="${l.no_oficio}">${l.no_oficio} — liberado ${formatearFechaCorta(l.liberado_en)}</option>`).join('')}
      </optgroup>`).join('');
}
