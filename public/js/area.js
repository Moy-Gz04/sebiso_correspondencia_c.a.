/* ═══════════════════════════════════════════════════
   ...
   El usuario de área solo ve los oficios turnados a su
   propia área. Puede: agregar observaciones, subir
   documentos de respuesta (doc3/doc4) y marcar el oficio
   como "atendido" (incluye reenvío tras un rechazo).
   ═══════════════════════════════════════════════════ */
...
function construirTarjeta(r, i) {
  ...
  const enConteo    = r.estatus === 'turnado';
...
/* ════════════════════════════════════════════════════
   MODAL: ATENDER OFICIO (turnado/rechazado → atendido)
   ════════════════════════════════════════════════════ */
...
async function guardarAtencion() {
  ...
  try {
    ...
    const res = await apiFetch(`${API}/oficios/${atendiendoId}`, {
      method: 'PUT',
      body:   fd
    });
    ...
    if (!res.ok) {
      ...
      throw new Error(d.mensaje || 'No se pudo guardar la atención del oficio.');
    }
    ...
    errEl.textContent = err.message || 'No se pudo guardar la atención del oficio.';
  } finally {
    btn.disabled  = false;
